 ...
 */
async function parseWithVertex(text, locale = "pt-BR") {
    const prompt = `
...
`;
    try {
        ...
        for (const op of opsRaw) {
            if (!op)
                continue;
            const tipo = op.tipo === "entrada" || op.tipo === "saida" ? op.tipo : undefined;
            const qtd = Number(op.quantidade || 0);
            const nome = String(op.produtoNome || "").trim();
            const preco = op.preco === null || op.preco === undefined
                ? null
                : Number(op.preco);
            const motivo = op.motivo === null || op.motivo === undefined
                ? null
                : String(op.motivo);
            if (!tipo || !nome || !Number.isFinite(qtd) || qtd <= 0)
                continue;
            parsedOps.push({
                tipo,
                quantidade: qtd,
        ...
        const answerText = typeof json.answer === "string" && json.answer.trim().length > 0
            ? json.answer.trim()
            : null;
        return {
 ...
 */
function looksLikeConsulta(t) {
    return /\b(quanto|tem|estoque|saldo)\b/i.test(t);
}
function looksLikeSugestao(t) {
    return /\b(sugira|sugere|recomende|recomenda|indique|indica|baixo estoque|repor|comprar mais|melhor marca|melhores produtos|aumentar a saida|aumentar a saída|produto de limpeza)\b/i.test(t);
}
function looksLikeBaixoEstoqueSugestao(t) {
...
function parseMovimento(text) {
    ...
    m = t.match(/\b(entrada|entrei|entrou|adiciona|adicionar|somar|somar ao estoque|repor|reabasteci|reabastecer|comprar para o estoque|comprei para o estoque|comprar)\s+(?:de\s+)?(\d+)\s+(.+?)(?:\s+a\s+(\d+[.,]?\d*))?$/i);
    if (m) {
        return {
            tipo: "entrada",
    ...
    m = t.match(/\b(saida|saída|venda|vender|vendi|vendemos|retirei|retirar|retiramos|baixar|baixei|baixamos|usei|usamos|consumi|consumimos|saiu do estoque)\s+(\d+)\s+(.+?)(?:\s+a\s+(\d+[.,]?\d*))?$/i);
    if (m) {
        return {
            tipo: "saida",
            quantidade: Number(m[2]),
...
async function registrarMovimento(tenantId, userId, tipo, produtoNome, quantidade, preco) {
    ...
    const atual = typeof data.quantidade === "number" ? data.quantidade : 0;
    // preço base: o da operação, senão o do produto, senão 0
    const precoBase = typeof preco === "number" && !Number.isNaN(preco)
        ? preco
...
async function gerarSugestaoLocal(tenantId) {
    ...
    const lista = snap.docs.map((d) => `${d.data().nome} (qtd ${d.data().quantidade})`);
    return `🔎 Itens com baixo estoque: ${lista.join(", ")}`;
}
/* =============================================
 *  🚀 chatRespond — onCall (IA + fallback)
 * ============================================= */
exports.chatRespond = functions
    .region("us-central1")
    ...
    try {
        ...
        if (operations.length > 0) {
            ...
            for (const op of operations) {
                const novoEstoque = await registrarMovimento(tenantId, userId, op.tipo, op.produtoNome, op.quantidade, op.preco);
                const precoTxt = op.preco != null && !Number.isNaN(op.preco)
                    ? ` a R$${op.preco.toFixed(2)}`
                    : "";
                if (op.tipo === "entrada") {
                    mensagens.push(`✅ Entrada: +${op.quantidade}x ${op.produtoNome}${precoTxt}. Estoque: ${novoEstoque}.`);
                }
                else {
                    mensagens.push(`✅ Saída: -${op.quantidade}x ${op.produtoNome}${precoTxt}. Estoque atual: ${novoEstoque}.`);
                }
            }
        ...
        if (looksLikeSugestao(tNorm)) {
            ...
            return {
                ok: true,
                intent: "analise",
                message: "No momento não consegui usar a IA para uma análise detalhada, " +
        ...
        if (looksLikeConsulta(tNorm)) {
            ...
            if (!nome) {
                return {
                    ok: true,
                    intent: "consulta",
                    message: 'Para consultar, você pode dizer: "quanto tem de coca lata?"',
                };
            }
            const snap = await admin
                .firestore()
        ...
        return {
            ok: true,
            intent: "geral",
            message: "Posso ajudar com **entrada**, **saída**, **consulta** e **sugestões de negócio**.\n" +
                "Exemplos:\n" +
