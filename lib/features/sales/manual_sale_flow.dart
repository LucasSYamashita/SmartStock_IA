...
final _catalogProvider = StreamProvider.autoDispose<List<Product>>((ref) {
...
class _ManualSaleCatalogPageState extends ConsumerState<ManualSaleCatalogPage> {
      ...
      body: async.when(
        data: (items) {
          final filtered = items.where((p) {
            ...
            return p.nome.toLowerCase().contains(s) ||
                p.categoria.toLowerCase().contains(s) ||
  ...
  Future<void> _finalizarVenda() async {
    ...
    final ids =
        _qty.entries.where((e) => e.value > 0).map((e) => e.key).toList();
    if (ids.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Nenhum item selecionado.')));
      return;
    }

    // Firestore whereIn suporta até 10: na prática, para catálogo pequeno ok.
    final chunk = ids.length > 10 ? ids.sublist(0, 10) : ids;
    ...
    for (final entry in _qty.entries) {
      ...
      final preco = precoAny is num ? precoAny.toDouble() : 0.0;
      total += preco * q;
